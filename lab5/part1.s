...
li t0, 0 # Counter variable for printing
li t1, 16 # One over highest number that can be displayed properly
li a1, 0 # Use HEX0 display

myloop:
    beq t0, t1, finished # Stop at 15
    mv a0, t0 # Load the number onto reg a0
    li a1, 0 # Keep 0 in a1 as subroutine modifies it
    call HEX_DISP # Show lower 4 bits of a0 onto HEX0
    call DELAY # Delay to see different values on display
    addi t0, t0, 1 # Next number to display
    j myloop

finished:
    li a0, 0x10 # Change 4th bit of a0 to 1
    li a1, 0 # Keep 0 in a1 as subroutine modifies it
    call HEX_DISP # Blank the hex this way
    j iloop
...
DELAY:
    li t2, 5000000 # Upper bound for delay
    dloop:
        addi t2, t2, -1 # Countdown
        bne t2, zero, dloop # Loop until counter hits 0
        ret                  
